...
def load_definition(def_path):
    ...
    # ---- 动态扫描所有回归配置（必须先初始化 reg_configs） ----
    reg_configs = []  # ✅ 显式初始化为空列表
    ...
    # ---- 从 config_raw 中提取每个回归自变量行的 C 列作为回归方法 ----
    for idx, row in config_raw.iterrows():
        a_col = str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else ''
        b_col = str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else ''
        c_col = str(row.iloc[2]).strip().lower() if pd.notna(row.iloc[2]) else ''
        reg_num = None
        if a_col.startswith('reg') and a_col.endswith('_ind'):
            reg_num = a_col.replace('_ind', '')
        elif b_col.startswith('reg') and b_col.endswith('_ind'):
            reg_num = b_col.replace('_ind', '')
        if reg_num:
            for reg_cfg in reg_configs:
                if reg_cfg.get('reg_num') == reg_num:
                    reg_cfg['method'] = 'r' if c_col == 'r' else 'python'
                    break
