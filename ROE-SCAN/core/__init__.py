...
def run_analysis(spss_path, def_path, output_path="ROE_Results", log_callback=print):
    ...
    df, meta, name_label_map = read_spss(spss_path, config['filter_var'], config['filter_val'])        
    ...
    try:
        ...
        brands, channel_df, kpi_df = extract_samples_from_full_table(
            full_table_df, config, name_label_map, log_callback=log_callback
        )
    ...
    qd_means, channel_means = seen_vs_noseen(
        df=df,
        x_vars=used_x_vars,
