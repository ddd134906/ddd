...
def load_definition(def_path):
    ...
    # ---- 特殊处理：多行竖排的 Sample_KPI_Keywords（修复版）----
    kpi_keywords = []
    kpi_types = []
    kpi_start_row = None

    # 遍历每一行，查找Sample_KPI_Keywords标题行（兼容A列/B列，模糊匹配）
    ...
    # ---- Factor_Target 读取（保留原有逻辑，仅优化容错、修正applymap、统一缩进）----
    try:
